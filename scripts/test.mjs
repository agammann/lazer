...
for (const name of ['engine', 'bitcoin', 'escrow', 'ethereum']) await build({ entryPoints: ['lib/' + name + '.ts'], bundle: true, platform: 'node', packages: 'external', format: 'esm', outfile: 'work/' + name + '.mjs' });
...
const result = spawnSync(process.execPath, ['--test', 'tests/engine.test.mjs', 'tests/api.test.mjs', 'tests/escrow.test.mjs', 'tests/ethereum.test.mjs'], { stdio: 'inherit' });
